#!/usr/bin/env python3
"""FastAPI server using trained PyTorch model"""
...
class ColorBranch(nn.Module):
    def __init__(self):
        ...
        self.block2=nn.Sequential(
            nn.Conv2d(32,3,3,2,1,bias=False),
            nn.BatchNorm2d(3),
            nn.LeakyReLU(0.2)
        )
...
if not os.path.exists(model_path):
    ...
    print(f"   Available files in assets:")
...
@app.get("/health")
async def health():
    return {"status": "ok", "classes": len(labels), "version": "2.0"}
